...
read or write Supabase, auth, tenancy or RLS data. The current root layout still
constructs a Supabase client, so local review needs non-production
`NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` placeholders or
equivalent protected preview env vars.
